    ...
    Alpine.data('adminAgendaCalendar', () => ({
        ...
        init() {
            ...
            var events = [
                ...
                { id:'m-{{ $m->id }}', title:@json($m->nama_rapat), start:'{{ \Carbon\Carbon::parse($m->tanggal)->format("Y-m-d") }}T{{ $m->waktu ? \Carbon\Carbon::parse($m->waktu)->format("H:i") : "00:00" }}', extendedProps:{ status:'{{ $m->status_rapat }}', tipe:'{{ $m->tipe_rapat === "Offline" ? "offline" : "online" }}', url:'{{ $m->tipe_rapat === "Offline" ? "#" : route("meeting.room", $m->id) }}', displayTime:'{{ \Carbon\Carbon::parse($m->tanggal)->translatedFormat("d M Y") }} - {{ $m->waktu ? \Carbon\Carbon::parse($m->waktu)->format("H:i") : "Sepanjang Hari" }}', deskripsi:@json($m->deskripsi_rapat ?? ''), nama_rapat:@json($m->nama_rapat), tanggal:'{{ $m->tanggal }}', waktu:'{{ $m->waktu ? \Carbon\Carbon::parse($m->waktu)->format("H:i") : "" }}', tipe_db:'{{ $m->tipe_rapat }}' }, backgroundColor:'{{ $m->tipe_rapat === "Offline" ? "#F59E0B" : ($m->status_rapat === "Berlangsung" ? "#10B981" : "#7c3aed") }}' },
                @endforeach
            ];
